...
--task 3: drop database 
--task 4: create new daatbase with tables FACULTY and AUDITORY_TYPE 
--next task: set options database create 
if db_id('TMPSOB_UNIVER') IS NOT NULL
    DROP DATABASE TMPSOB_UNIVER;
if db_id('S_univer') is not NULL
...
PRIMARY
    (
    NAME = S_univer1,  
    FILENAME = 'c:\UniverData\S_univer_dat1.mdf',  
    SIZE = 5MB,  
    MAXSIZE = 10,  
    FILEGROWTH = 1),  
    ( NAME = S_univer2,  
    FILENAME = 'c:\UniverData\S_univer_dat2.ndf',  
    SIZE = 5MB,  
    MAXSIZE = 10,  
    FILEGROWTH = 10%),
...
(
    NAME=S_univer11,
    FILENAME='c:\UniverData\S_univer11.ndf',
    size=10mb,
    maxsize=15,
    FILEGROWTH=1mb
),
(
    name=S_univer12,
    FILENAME='c:\UniverData\S_univer12.ndf',
    size=5mb,
    maxsize=5,
FILEGROWTH=1MB),
FILEGROUP G2
(
    NAME=S_univer21,
    FILENAME='c:\UniverData\S_univer21.ndf',
    size=5mb,
    maxsize=10,
FILEGROWTH=1MB),
(
    name=S_univer22,
    FILENAME='c:\UniverData\S_univer22.ndf',
    size=2mb,
    maxsize=5,
FILEGROWTH=1MB)   
...
LOG ON   
   (NAME = Archlog1,  
    FILENAME = 'c:\UniverData\S_univer_log1.ldf',  
    SIZE = 5MB,  
    MAXSIZE = UNLIMITED,  
    FILEGROWTH = 20)
...
if OBJECT_ID('TEACHER') is not NULL
    drop TABLE TEACHER;
if OBJECT_ID('PULPIT') is not NULL
    drop TABLE PULPIT;


if OBJECT_ID('AUDITORIUM') is not NULL
    drop TABLE AUDITORIUM ;

if OBJECT_ID('AUDITORIUM_TYPE') is not NULL
